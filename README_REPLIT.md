...
## Вкладка «График застройки» в календаре

На странице календаря (`/calendar`) рядом с режимами «Месяц / Список / Гантт»
добавлена кнопка **«График застройки»**. Она показывает только те выставки
выбранного месяца, у которых есть хотя бы одна сделка на стадиях из
`BUILD_SCHEDULE_STAGE_IDS` (по умолчанию `8`, `9`, `WON` — «Подписываем
договор», «Строим», «Проект завершён»).

Строка каждой выставки растёт по высоте под количество сделок и содержит:

- верхнюю полосу — общий период (монтаж/выставка/демонтаж), обрезанный по месяцу;
- стек цветных полосок: жёлтая = «Подписываем договор», синяя = «Строим»,
  зелёная = «Проект завершён», серая = прочая стадия из whitelist;
- клик по сделке открывает Bitrix24 (если задан `VITE_BITRIX_PORTAL_URL`),
  иначе — карточку выставки.

### Настройки сборки клиента

| Переменная | По умолчанию | Назначение |
| --- | --- | --- |
| `VITE_BUILD_SCHEDULE_STAGE_IDS` | `8,9,WON` | Whitelist стадий сделок для вкладки «График застройки». Поддерживает tail-сопоставление (`C1:8` тоже подойдёт). |
| `VITE_BITRIX_PORTAL_URL` | _(не задана)_ | Origin портала Bitrix24, используется для построения ссылок на сделку из полосок графика. |

Изменения этих переменных применяются на этапе сборки клиента
(`npm run build` / Vite), серверного перезапуска не требуют.
